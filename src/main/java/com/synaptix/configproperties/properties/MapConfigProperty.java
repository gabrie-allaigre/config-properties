...
public class MapConfigProperty<E, F> implements IConfigProperty<Map<E, F>> {
    ...
    public Map<E, F> setProperty(Properties properties, IComponent component) {
        Map<E, F> map = properties.stringPropertyNames().stream().filter(p -> p.startsWith(key + separator) && p.length() > (key.length() + separator.length())).map(p -> set(properties, p)).collect(supplier, (m, e) -> m.put(e.getKey(), e.getValue()), Map::putAll);
